...
* **`GrossGrams`**: The physical weight printed on the product label (e.g., "500 GMS", "1 KG"). Resolved via a two-tier priority chain: **(1)** `VariantGrossOverrides[v.Title]` — per-variant manual override for variants whose titles lack standard gram/kg patterns (e.g., `"30 SERV"`); **(2)** regex extraction via `reLabelGrams`/`reLabelKg` scanning `variant.Title` and `product.Title` only — never `body_html`. Defaults to `0` for capsule-only products or when neither override nor regex yields a value. NOT used in cost calculations — exists solely for frontend transparency. When `0`, the frontend displays "—".
