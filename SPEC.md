...
* **`ActiveGrams`**: The total active ingredient mass in grams. This is the denominator for `CostPerGram` and `EffectiveCost` calculations. Populated by the Hybrid Engine's priority chain: variant override > product override (`ForceTotalGrams`) > regex pipeline. For "Pure Powder" products (no dirty keywords), if a label weight (GrossGrams) was found and mass was regex-resolved (not override), ActiveGrams is set equal to GrossGrams.
* **`GrossGrams`**: The physical weight printed on the product label (e.g., "500 GMS", "1 KG"). Extracted independently from `variant.Title` and `product.Title` only — never from `body_html`. Defaults to `0` for capsule products or when no label weight is found. NOT used in cost calculations — exists solely for frontend transparency. When `0`, the frontend displays "—".
