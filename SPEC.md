...
* **Math Engine (`internal/parser/analyzer.go`):** Implements a **Hybrid Catalog/Regex Engine** with three-tier mass resolution. `AnalyzeProduct()` returns `[]models.Analysis` — a slice containing one entry per valid variant. At the top of the variant loop, the engine checks `VendorConfig.VariantBlocklist` against `v.Title` (case-insensitive); matched variants are skipped via `continue`. For mass extraction, the engine evaluates a strict priority chain: **(1)** `spec.VariantOverrides[v.Title]` — per-variant override takes highest priority, bypassing both the product-level override and regex; **(2)** `spec.ForceTotalGrams` — product-level override bypasses regex; **(3)** standard regex pipeline (`reGrams`, `reKg`, `reMg`, `reCount`, `reServing`). The `rePack` regex (pack multiplier) always runs regardless of override source. `totalGrams = baseMass * packMultiplier`. If the override has a `ForceType`, it is used directly; otherwise, string-matching logic determines the product type. When a vendor has a `GlobalSubscriptionDiscount > 0`, a second synthetic "Subscribe & Save" entry is emitted for each variant with the discounted price, `" (Subscribe & Save)"` appended to its `Name`, and `IsSubscription: true`. The function evaluates ALL valid variants (no min-cost gatekeeper). Returns `nil` when the product has no analyzable variants. `cmd/main.go` flattens all returned slices into the master report via `append(report, analyses...)`.
* **Triage Engine (`internal/parser/analyzer.go`):** After mass extraction and before appending each `Analysis` entry, the analyzer runs a dirty-data detection pass. If the mass was NOT resolved by an override (`usedOverrideForMass == false`), the engine scans `displayName`, `p.Handle`, and `p.Title` (case-insensitive) against `dirtyKeywords` — a hardcoded slice of red-flag strings: `flavor`, `watermelon`, `berry`, `punch`, `orange`, `lemon`, `mango`, `grape`, `apple`, `blend`, `complex`, `with`, `+`, `gumm`, `chew`, `bundle`. On a match, the entry is emitted with `NeedsReview: true` and `ReviewReason: "Detected dirty keyword: <matched>"`. Both one-time and synthetic subscription entries inherit the same flag. `cmd/main.go` extracts all flagged entries into a `reviewQueue` slice and writes it to `data/needs_review.json` (pretty-printed) after saving the main report. The triage is intentionally aggressive — it flags for human review, not rejection.
* **Audit Gap Detector (`internal/parser/audit.go`):** `AuditProduct()` runs the same extraction pipeline as `AnalyzeProduct()` but never silently discards products. For each product that passes the supplement keyword filter and vendor blocklist but yields `totalGrams == 0`, it returns an `AuditResult` struct listing the product handle, what data was extracted (mg, count, grams), what is missing, and a suggested `vendor_rules.json` override snippet using the `forceTotalGrams` / `forceServingMg` format. When both mg and count are extracted, the suggestion pre-computes `forceTotalGrams = mg * count / 1000`. `FormatAuditReport()` groups results by vendor and renders them as a human-readable stdout report. Triggered by the `-audit` CLI flag.
